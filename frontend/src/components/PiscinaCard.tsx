...
import { TbDropletPause, TbPlugOff } from 'react-icons/tb';
...
export const PiscinaCard = ({ piscina, onClick, layout, isFullscreen }: PiscinaCardProps) => {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      {/* Card Header: Last Sync - MOVIDO FUERA DEL FILTRO */}
      <div
        style={{
          position: 'absolute',
          top: 0,
          ...
          alignItems: 'center',
          justifyContent: 'center',
          zIndex: 20 // Mayor que el overlay
        }}
      >
      ...
      {/* Nombre de la piscina - MOVIDO FUERA DEL FILTRO */}
      <div
      ...
      {/* Overlay para estado Desconectado */}
      {!piscina.online && (
        <div
          style={{
            position: 'absolute',
        ...
        >
          ...
          >
            <TbPlugOff style={{ fontSize: '1.2rem', color: '#555' }} />
            <span style={{
              color: '#555',
              fontWeight: '600',
              fontSize: '0.95rem',
      ...
      {/* Contenedor principal con filtro si está offline */}
      <div style={{
        filter: !piscina.online ? 'grayscale(100%) opacity(0.5)' : 'none',
